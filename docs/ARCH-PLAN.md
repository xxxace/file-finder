...
2. **P2 的"删备份保留策略不用管"这个判断要补一句。** `cacheBackup()` 的文件名是 `searchCache-YYYYMMDD.db`（`nedb.ts:91` 的 `cachePath.replace('.db', '-YYYYMMDD.db')`，只替第一个 `.db`）—— **同一天多次备份会互相覆盖**。本次就发生了：第一次重启的 107 MB 备份被第二次重启的 360 KB 覆盖了。不影响正确性（2024 那份 123 MB 还在，且被覆盖的本来就是要丢弃的旧格式数据），但要知道"想留住某个时刻的备份就得当天先拷走"。
